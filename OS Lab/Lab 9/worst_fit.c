...
void worst_fit_allocation(Hole holes[], int nh, Process processes[], int np) {
    for (int i = 0; i < np; i++) {
        int worst_fit_index = -1;
        int largest_hole_size = -1;

        for (int j = 0; j < nh; j++) {
            if (holes[j].size >= processes[i].size && holes[j].size > largest_hole_size) {
                worst_fit_index = j;
                largest_hole_size = holes[j].size;
            }
        }

        if (worst_fit_index != -1) {
            processes[i].allocated_hole = holes[worst_fit_index].id;
            holes[worst_fit_index].size -= processes[i].size;
            holes[worst_fit_index].allocated = 1;
        }
    }
}
...
int main() {
    ...
    worst_fit_allocation(holes, nh, processes, np);
